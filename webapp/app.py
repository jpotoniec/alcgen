...
def config(key_prefix: str, disabled: bool, closed: bool) -> RandomGuideConfiguration | None:
    ...
    n_conjuncts = st.slider("Conjuncts count", min_value=min_n_conjuncts, max_value=5, value=(min_n_conjuncts, 3),
    ...
    n_disjuncts = st.slider("Disjuncts count", min_value=2, max_value=5, value=(2, 2), step=1,
                            disabled=disabled or p_disjuncts <= 0.0, key=f"{key_prefix}_n_disjuncts")
    can_use_existential = depth > 0
    n_existential = st.slider("Existential restrictions count", min_value=0, max_value=5, value=(0, 3), step=1,
                              disabled=disabled or not can_use_existential, key=f"{key_prefix}_n_existential")
    n_roles = st.slider("Maximal number of roles", min_value=1, max_value=5, value=1, step=1,
...
depth = st.slider("ABox depth", min_value=0, max_value=5, step=1, value=2)
...
seed = st.number_input("Random seed", min_value=0, max_value=(1 << 32) - 1, value=secrets.randbits(32),
                       help="The seed to be used in the random number generator. The same seed with the same configuration will yield the same formula.")
